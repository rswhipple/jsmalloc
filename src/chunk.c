...
t_chunk* create_top_chunk(t_page* page) {
    ...
    page->base.top_chunk = (t_base_chunk*)chunk;
    return chunk;
}

t_tiny_chunk* create_top_tiny_chunk(t_fpage* page) {
    t_tiny_chunk* tiny = (t_tiny_chunk*)FASTPAGE_SHIFT(page);
    tiny->parent = (t_page*)page;
    tiny->base.next = NULL;
    tiny->base.data = (void*)MEMORY_SHIFT(TINY_CHUNK_SHIFT(tiny), 0);
    page->base.top_chunk = (t_base_chunk*)tiny;
...
t_tiny_chunk* split_tiny_chunk(t_fpage* page, t_tiny_chunk* prev_chunk) {
    t_tiny_chunk* tiny;
    tiny = (t_tiny_chunk*)MEMORY_SHIFT(prev_chunk->base.data, page->chunk_size);
    ...
    // add to linked list
    if (prev_chunk) {
        prev_chunk->base.next = (t_base_chunk*)tiny;
    }
